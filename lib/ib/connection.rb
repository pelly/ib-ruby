...
module IB
  ...
  class Connection
    ...
    class CantGetNextValidId < StandardError
    end
    ...
    def initialize host: '127.0.0.1',
      ...
      ensure_next_valid_id_subscription

      ensure_trasmission_of_next_valid_id if connect
    ...
    def ensure_next_valid_id_subscription
      unless @next_valid_id_sub_id && subscribers[translate_message_classes[:NextValidId].first][@next_valid_id_sub_id]
        @next_valid_id_sub_id = self.subscribe(:NextValidId) do |msg|
    ...
    def connect
      ...
      self.socket = socket_factory.open(@host, @port) # raises  Errno::ECONNREFUSED  if no connection is possible
    ...
    def translate_message_classes(what)
      case
    ...
    def subscribe *args, &block
      @subscribe_lock.synchronize do
        ...
        args.each do |what|
          message_classes = translate_message_classes(what)
    ...
    def subscribers_count
      @subscribe_lock.synchronize do
        Hash[self.subscribers.map { |name, subs| [name, subs.length] }]
      end
    end

    def intercept_with(&block)
      @msg_intercept = block
      self
    end

    def with_message_interceptor(&block)
      prev_interceptor = @msg_intercept
      begin
        @msg_intercept = block
        yield
      ensure
        @msg_intercept = prev_interceptor
      end
    end
    ...
    def intercept_raw_messages(messages)
      begin
        @msg_intercept.call(messages) if @msg_intercept
        messages
      rescue Exception => e
    ...
    def process_message
      ...
      socket.decode_message(intercept_raw_messages(socket.recieve_messages)) do |the_decoded_message|
